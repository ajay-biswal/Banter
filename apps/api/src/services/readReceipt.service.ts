 ...
 */
export const markMessagesDelivered = async (messageIds: string[], userId: string) => {
 ...
 */
export const markMessagesRead = async (messageIds: string[], userId: string, conversationId: string) => {
  // Validate message IDs
  const validMessageIds = messageIds.filter(id => Types.ObjectId.isValid(id));
  if (validMessageIds.length === 0) {
    throw new AppError('No valid message IDs provided', 400);
 ...
 */
export const markConversationMessagesRead = async (conversationId: string, userId: string, lastReadMessageId?: string) => {
