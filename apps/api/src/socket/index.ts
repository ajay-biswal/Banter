...
import { PresenceStore } from './presence/presence.interface.js';
...
export const initSocket = (server: any) => {
  ...
  if (env.SOCKET_REDIS_ENABLED === 'true' || env.SOCKET_REDIS_ENABLED === true) {
    ...
    const pubClient = (presenceStore as RedisPresenceStore).getClient();
  ...
  io.on('connection', (socket: Socket) => {
    ...
    const emitPresence = (target: string, presenceUserId: string, status: 'online' | 'offline', lastSeen?: string | null) => {
    ...
    const notifyConversationParticipants = async (status: 'online' | 'offline', lastSeen?: string | null) => {
    ...
    if (presenceStore instanceof RedisPresenceStore) {
      presenceStore.userConnected(userId).catch(err => 
        console.error(`Error marking user ${userId} as connected:`, err)
      );
    } else {
    ...
    socket.on('disconnect', () => {
      ...
      if (presenceStore instanceof RedisPresenceStore) {
        presenceStore.userDisconnected(userId).catch(err => 
          console.error(`Error marking user ${userId} as disconnected:`, err)
        );
      } else {
            ...
            if (participantId !== userId) {
              ...
              const getParticipantPresence = async () => {
                if (presenceStore instanceof RedisPresenceStore) {
                  ...
                  return (presenceStore as any).getUserPresence(participantId);
                }
              };
              ...
              if (participantPresence) {
                ...
                socket.emit(
                  participantPresence.online ? 'user:online' : 'user:offline',
                  participantId
                );
              }
            }
